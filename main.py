...
def write_metadata(folders, path="Solution"):
    """Writes metadata for each folder containing audio files."""
...
# Worst Fit Linear Algorithm
def worst_fit_linear(audios, path):
    """Distributes audio files into folders using the worst fit linear strategy."""
    folders = {}
...
def worst_fit_decreasing_linear(audios, path):
    """Distributes audio files into folders using the worst fit decreasing linear strategy."""
...
def worst_fit_pq(audios, path):
    """Distributes audio files into folders using the worst fit strategy with a priority queue."""
    pq = []
    folders = {}
    folder_contents = {}  # To store the files in each folder -> for writing the metadata
...
def worst_fit_decreasing_pq(audios, path):
    """Distributes audio files into folders using the worst fit decreasing strategy with a priority queue."""
    sorted_audios = sorted(audios, key=lambda x: x[1], reverse=True)
    return worst_fit_pq(sorted_audios, path)

# First Fit Decreasing Algorithm
def first_fit(audios, path):
    """Distributes audio files into folders using the first fit decreasing strategy."""
    folders = {}
    folder_contents = {}
    sorted_audios = sorted(audios, key=lambda x: x[1], reverse=True)
...
def folder_filling(audios, path):
    """Distributes audio files into folders using a dynamic programming approach."""
    memo = {}
    ...
    def dp(remaining_duration, num_files):
        """Dynamic programming function to calculate maximum duration."""
    ...
    def backtracking(remaining_duration, num_files):
        """Backtracks to find which files were selected."""
        selected_files = []
...
def best_fit(audios, path):
    """Distributes audio files into folders using the best fit strategy."""
    folders = {}
    folder_contents = {}
...
def best_fit_decreasing(audios, path):
    """Distributes audio files into folders using the best fit decreasing strategy."""
    sorted_audios = sorted(audios, key=lambda x: x[1], reverse=True)
    return best_fit(sorted_audios, path)

def execute_strategy(strategy, audios, path, max_folder_duration):
    """Executes the selected strategy for distributing audio files into folders."""
...
def reset():
    """Resets the input fields and result label."""
    folder_path_entry.delete(0, tk.END)
